...
import numpy as np
import pandas as pd
from training_dt import DecisionTransformer 
...
def evaluate(task_id, dt_model, file, state_type, reward_type):
    formula = formulas[task_id]

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # 1. Carica il modello
    model = DecisionTransformer(action_dim=4, state_type=state_type, state_dim=2).to(device)
    ...
    print(f"Inizio eval per task {task_id} | Stato: '{state_type}' | Reward: '{reward_type}'...")
    ...
    for start_pos in possible_starts:
        ...
        for run in range(num_test):
            ...
            current_state = obs[1] if isinstance(obs, list) else obs
            ...
            # Memoria degli oggetti visitati (Serve solo se usiamo l'Oracolo)
            visited_items = set()
            ...
            while not done:
                ...
                states_arr = np.array(states_window, dtype=np.float32)
                if state_type == 'image':
                    states_arr /= 255.0

                s_tensor = torch.tensor(states_arr).unsqueeze(0).to(device)
                ...
                # ------------------------------------------------------------------
                # LOGICA CONDIZIONATA AL TIPO DI REWARD (ORACOLO)
                # ------------------------------------------------------------------
                if reward_type == "three_value":
                    ...
                    # Oracolo per Task "buggati"
                    if task_id in [6, 7]:
                        if 'pickaxe' in visited_items and 'lava' in visited_items:
                            reward = 100.0
                            done = True
                    elif task_id in [2, 8, 9]:
                        if curr_pos == (3, 3) and 'pickaxe' in visited_items:
                            reward = 100.0
                            done = True
                # ------------------------------------------------------------------
                ...
                # Update RTG (sia per three_value che distance)
            ...
            # Soglia di successo unificata a >= 99.0 per sicurezza su arrotondamenti
            success_flag = 1 if ep_reward >= 99.0 else 0 
...
def eval_all(state_type, reward_type, task_id):
    ...
    if os.path.exists(dt_path) and not os.path.exists(csv_path): 
        evaluate(task_id, dt_path, csv_path, state_type, reward_type)
