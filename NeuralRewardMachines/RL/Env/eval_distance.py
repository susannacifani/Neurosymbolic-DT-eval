...
def evaluate_distance(task_id, dt_model, file):
    ...
    # NOTA: Qui dovrai aggiungere reward="distance" se stai testando quella configurazione
    ...
    for start_pos in possible_starts:
        print(f"\n--- Valutazione Posizione: {start_pos} ---")
        ...
        for run in range(num_test):
            ...
            # # Memoria degli oggetti visitati
            # visited_items = set()
            # if tuple(env._agent_location) == (1, 1): visited_items.add('pickaxe')
            # if tuple(env._agent_location) == (3, 3): visited_items.add('lava')
            
            # # Identifichiamo i task "buggati" (quelli con Global Avoidance)
            # # Nelle tue formule LTL, sono gli ID 6, 7, 8, 9 (Task 7, 8, 9, 10)
            ...
            while not done:
                ...
                # ------------------------------------------------------------------
                # # Aggiorniamo gli oggetti visitati
                # curr_pos = tuple(env._agent_location)
                # if curr_pos == (1, 1): visited_items.add('pickaxe')
                # if curr_pos == (3, 3): visited_items.add('lava')
                
                # ------------------------------------------------------------------
                # L'ORACOLO UNIVERSALE
                # Se è un task 0-5, il DFA originale fa già tutto da solo (ci darà lui 100 e done=True).
                # Se è un task 6-9 e abbiamo preso gli oggetti base senza morire, forziamo la vittoria!
                # ------------------------------------------------------------------
                ...
                # ATTENZIONE: Se usi reward="distance", la logica di update dell'RTG potrebbe
                # dover cambiare, a seconda di come hai salvato i dati nell'HDF5.
                # Per ora teniamo la scala / 100.0 (se il massimo cumulativo atteso è ~100)
            ...
            # Se la reward cambia, forse 100 non è più la soglia di vittoria.
            # Dovresti controllare 'info' se l'ambiente ti dice se ha vinto o no
            # Per ora lasciamo la logica a soglia >= 100
...
def eval_all_distance(state_type, reward_type, task_id):
    ...
    if not os.path.exists(csv_path): evaluate_distance(task_id, dt_path, csv_path)
